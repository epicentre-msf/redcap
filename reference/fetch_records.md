...
data frame containing records for one or more REDCap instruments.
...
- events:
  ...
  Character vector of events to fetch. Must correspond to the selected
  `forms`. Set to `NULL` (the default) to fetch all events corresponding
  to the selected form(s).
...
- fields:
  ...
  Character vector of fields (i.e. variables) to fetch. Set to `NULL`
  (the default) to fetch all fields corresponding to the selected
  form(s).
...
A
[`tibble`](https://tibble.tidyverse.org/reference/tbl_df-class.html)-style
data frame containing the requested records
...
If argument `rm_empty` is `TRUE` (the default), `fetch_records()` will
filter out such rows. The check for empty rows is based only on fields
that are specific to the form(s) specified in argument `forms` — i.e. it
excludes the record ID field, and generic fields like
